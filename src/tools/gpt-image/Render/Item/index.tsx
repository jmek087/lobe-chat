...
const ImageItem = memo<GPTImageItem & { index: number; messageId: string }>(
  ({ prompt, messageId, imageId, previewUrl, index, size, quality }) => {
    const { t } = useTranslation('tool');
    const { styles } = useStyles();

    const [edit, setEdit] = useState(false);
    const loading = useChatStore(chatToolSelectors.isGptImageGenerating(messageId + prompt));
